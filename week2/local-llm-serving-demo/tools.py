...
class ToolRegistry:
    ...
    def _register_default_tools(self):
        ...
        self.register_tool(
            ...
            description="Get the current local date and time for a city or timezone",
            parameters={
                "type": "object",
                "properties": {
                    "location": {
                        "type": "string",
                        "description": "City and country, e.g. 'Vancouver, Canada'. Use this when the user asks for a city's local time."
                    },
                    "timezone": {
                        "type": "string",
                        "description": "IANA timezone name (e.g. 'America/Vancouver'). Optional if location is provided.",
                        "default": "UTC"
                    }
                },
                "required": []
    ...
    @staticmethod
    def _resolve_timezone_from_location(location: str) -> Optional[str]:
        """Resolve IANA timezone from city name via Open-Meteo geocoding."""
        try:
    ...
    @staticmethod
    def get_current_time(timezone: str = "UTC", location: Optional[str] = None) -> Dict:
        """
        Get current date and time in specified timezone using zoneinfo (Python 3.9+).
        If location is given, resolve its IANA timezone via geocoding (takes precedence).
        """
        from datetime import datetime
        ...
        if location:
            geo_tz = ToolRegistry._resolve_timezone_from_location(location)
            if geo_tz:
                tz_name = geo_tz
            else:
